...
const TaskList = ({ taskList, handleOnEdit, handleOnDelete, archiveCompletedTask, handleEditfn }) => {
  ...
  let tasks = taskList.map((task) => {
    ...
    if (task.completed === true) {
      return <ArchivedTask key={task._id} id={task._id} taskName={task.taskTitle} description={task.taskDescription} dueDate={task.dueDate} dateAdded={task.dateAdded} level={task.level}/>
    } else if (task.level <= 3) {
      return <LowerLevelTask key={task._id} id={task._id} taskName={task.taskTitle} description={task.taskDescription} dueDate={task.dueDate} dateAdded={task.dateAdded} level={task.level} handleOnEdit={handleOnEdit} handleOnDelete={handleOnDelete} daysOld={task.daysOld} archiveCompletedTask={archiveCompletedTask} handleEditfn={handleEditfn}/>;
    } else {
