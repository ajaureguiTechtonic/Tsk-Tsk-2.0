...
const TaskList = ({ taskList, handleOnEdit, handleOnDelete, archiveCompletedTask, handleEditfn }) => {
  ...
  let tasks = taskList.map((task) => {
    ...
    if (task.completed === true) {
      return <ArchivedTask key={task._id} id={task._id} taskName={task.taskTitle} description={task.taskDescription} dueDate={task.dueDate} dateAdded={task.dateAdded} level={1}/>;
