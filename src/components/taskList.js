...
class TaskList extends Component {
  ...
  calcDaysOld(dateAdded, currentDate) {
  ...
  render() {
    let tasks = this.props.taskList.map((task, i) => {
        let level = this.sortTasks(task);
        let currentDate = new Date().getTime();
        var daysOld = this.calcDaysOld(task.dateAdded, currentDate);

        if (level < 3) {
          return <LowerLevelTask key={task.taskID}  taskName={task.taskName} description={task.description} dueDate={task.dueDate} dateAdded={task.dateAdded} level={level} handleOnClick={this.props.handleOnClick} id={task.taskID} daysOld={daysOld}/>;

        } else {
          daysOld = this.calcDaysOld(task.dateAdded, new Date().toDateString());
          var daysPastDue = this.calcDaysOld(task.dueDate, new Date().toDateString());

          return <HigherLevelTask key={task.taskID} taskName={task.taskNAme} description={task.description} dueDate={task.dueDate} level={level} handleOnClick={this.props.handleOnClick} id={task.taskID} daysOld={daysOld} daysPastDue={daysPastDue}/>;
        }
      });
...
// console.log(taskList, 'in TASKLIST');
// console.log(props);
