...
const TaskList = ({ taskList, handleOnEdit, handleOnDelete, handleDeleteTask }) => {
  ...
  let tasks = taskList.map((task, i) => {
    ...
    if (level < 3) {
      ...
      return <LowerLevelTask key={task.taskID} id={task.taskID}  taskName={task.taskName} description={task.description} dueDate={task.dueDate} dateAdded={task.dateAdded} level={level} handleOnEdit={handleOnEdit} handleOnDelete={handleOnDelete} handleDeleteTask={handleDeleteTask} daysOld={daysOld} />;
    } else {
      daysOld = calcDaysOld(task.dateAdded, new Date().toDateString());
      var daysPastDue = calcDaysOld(task.dueDate, new Date().toDateString());

      return <HigherLevelTask key={task.taskID} id={task.taskID} taskName={task.taskNAme} description={task.description} dueDate={task.dueDate} level={level} handleOnEdit={handleOnEdit} handleOnDelete={handleOnDelete} handleDeleteTask={handleDeleteTask} daysOld={daysOld} daysPastDue={daysPastDue}/>;
    }
  });
